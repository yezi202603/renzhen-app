...
} from '/var/task/workspace/renzhen-app/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer, withActiveSpan } from '/var/task/workspace/renzhen-app/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('/var/task/workspace/renzhen-app')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return withActiveSpan(tracer, 'Next.js Server Handler', async (span) => {
      span?.setAttributes({
        ...
        monorepo: true,
        cwd: '/var/task/workspace/renzhen-app',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('/var/task/workspace/renzhen-app/.netlify/dist/run/handlers/server.js')
